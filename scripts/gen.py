...
def person_pixels(arm="down", blink=False):
    p = set()
    # head (8,0)-(19,9); eyes = white 2x2 gaps (never blinked away)
    for y in range(10):
        for x in range(8, 20):
            if y in (4, 5) and (11 <= x <= 12 or 17 <= x <= 18):
                continue
            p.add((x, y))
    # neck (12,10)-(15,12)
    ...
    for y in range(11, 24):
        for x in range(7, 20):
            p.add((x, y))
    # arms: hang (down), one raised (wave), both raised (wave2)
    # raised arm runs from the shoulder (y11) up past the head top (y-2):
    # long, continuous, and clear of the greeting text above
    if arm == "wave2":
        for y in range(-2, 12):
            for x in range(1, 6):
                p.add((x, y))
    ...
    if arm == "down":
        ...
        for y in range(-2, 12):
            for x in range(21, 26):
                p.add((x, y))
    # legs (6-9,24-27) & (17-20,24-27) — wide stance, symmetric about x=13
    for y in range(24, 28):
        for x in range(6, 10):
            p.add((x, y))
        for x in range(17, 21):
            p.add((x, y))
    # feet rows 28-29: (5-10) & (16-21) — sole at y29 lands on ground y42
...
def draw_hero(frame):
    ...
    for y in range(0, 47, 2):
        put(px, w, h, 40, y)
    # turn-taking: the person greets with one hand, the mirror answers with both
    real = person_pixels("wave" if frame == 1 else "down")
    refl = person_pixels("wave2" if frame == 2 else "down")
    for sx, sy in real:
        put(px, w, h, sx + 4, sy + 17)
    for sx, sy in refl:
        put(px, w, h, 53 + (27 - (sx + 4)), sy + 17)
    if frame == 1:
...
# ---------------------------------------------------------------------------
# dev-icons.gif — 560x64 long strip, 5 x 48px animated icons, space-between
...
def icon_btc(px, w, h, ox, oy, frame):
    ...
    for y in range(48):
        for x in range(48):
            ...
            d2 = ex * ex + dy * dy
            if 361 <= d2 <= 441:                     # thin clean ring r19-21
                if not narrow and 2 <= dx <= 3 and dy <= -19:  # highlight notch
                    continue
                put(px, w, h, ox + x, oy + y)
    if narrow:
        vline(px, w, h, ox, oy, 23, 21, 27)          # slim ₿ while flipping
        vline(px, w, h, ox, oy, 24, 21, 27)
    else:
        # ₿: 4px stem + true half-circle arcs + mid bar (rounded, not blocky)
        fill(px, w, h, ox, oy, 22, 14, 25, 34)       # stem
        for y in range(48):
            for x in range(24, 40):
                if (x - 24) ** 2 + (y - 19) ** 2 <= 36 and y <= 24:    # upper arc
                    put(px, w, h, ox + x, oy + y)
                if (x - 24) ** 2 + (y - 29) ** 2 <= 36 and y >= 24:    # lower arc
                    put(px, w, h, ox + x, oy + y)
        fill(px, w, h, ox, oy, 24, 24, 28, 25)       # mid bar
    ...
    for y in range(10, 36):                          # rounded head frame
        for x in range(11, 37):
            ...
            if (x in (11, 36) or y in (10, 35)) and not corner:
                put(px, w, h, ox + x, oy + y)
    for y in range(14, 32):                          # inner screen frame
        for x in range(14, 34):
            if x in (14, 33) or y in (14, 31):
                put(px, w, h, ox + x, oy + y)
    if blink:                                        # eyes -> closed lines
        hline(px, w, h, ox, oy, 17, 20, 21)
        hline(px, w, h, ox, oy, 27, 30, 21)
    else:                                            # 4x5 eyes + highlight
        for y in range(18, 23):
            for x in range(17, 21):
                if not (x == 17 and y == 18):
                    put(px, w, h, ox + x, oy + y)
            for x in range(27, 31):
                if not (x == 27 and y == 18):
                    put(px, w, h, ox + x, oy + y)
    fill(px, w, h, ox, oy, 21, 27, 26, 28)           # mouth


def icon_taiji(px, w, h, ox, oy, frame):
    # yin-yang: smooth S-curve divider, rotating clockwise 90° per frame
    ...
    for y in range(48):
        for x in range(48):
            dx, dy = x - c, y - c
            d2 = dx * dx + dy * dy
            if 361 <= d2 <= 441:                     # thin ring r19-21
                put(px, w, h, ox + x, oy + y)
                continue
            if d2 > 400:
                continue
            if f == 0:      rx, ry = dx, dy
            elif f == 1:    rx, ry = dy, -dx
            elif f == 2:    rx, ry = -dx, -dy
            else:           rx, ry = -dy, dx
            # smooth S divider: boundary = 2.5*sin(ry*pi/24)
            bound = 2.5 * _math.sin(ry * _math.pi / 24)
            black = rx < bound
            # large fish eyes, centered in each half
            w_eye = -12 <= rx <= -8 and -8 <= ry <= -4
            b_eye = 8 <= rx <= 12 and 4 <= ry <= 8
            if (black and not w_eye) or b_eye:
                put(px, w, h, ox + x, oy + y)


def icon_term(px, w, h, ox, oy, frame):
    # terminal window: title bar, big >_ prompt, blinking block cursor
    box(px, w, h, ox, oy, 8, 8, 39, 39)              # window frame
    hline(px, w, h, ox, oy, 8, 39, 10)               # title bar
    fill(px, w, h, ox, oy, 12, 13, 14, 14)           # title dots
    fill(px, w, h, ox, oy, 18, 13, 20, 14)
    fill(px, w, h, ox, oy, 24, 13, 26, 14)
    for i in range(12):                              # big prompt > (3px thick)
        put(px, w, h, ox + 13 + i, oy + 13 + i)
        put(px, w, h, ox + 14 + i, oy + 13 + i)
        put(px, w, h, ox + 15 + i, oy + 13 + i)
        put(px, w, h, ox + 13 + i, oy + 35 - i)
        put(px, w, h, ox + 14 + i, oy + 35 - i)
        put(px, w, h, ox + 15 + i, oy + 35 - i)
    if frame % 2 == 0:                               # cursor: block over underscore
        fill(px, w, h, ox, oy, 29, 28, 35, 32)
    else:
        hline(px, w, h, ox, oy, 29, 35, 31)
        hline(px, w, h, ox, oy, 29, 35, 32)


def icon_gear(px, w, h, ox, oy, frame):
    # gear: thick ring + 8 small teeth rotating 22.5° per frame (smooth spin)
    c = 24
    import math as _math
    for y in range(48):
        for x in range(48):
            dx, dy = x - c, y - c
            d2 = dx * dx + dy * dy
            if 169 <= d2 <= 361:                     # ring r13-19 (no spokes)
                put(px, w, h, ox + x, oy + y)
    for k in range(8):                               # teeth on radius ~19.5
        a = _math.radians(k * 45 + frame * 22.5)
        tx = 24 + int(19.5 * _math.cos(a))
        ty = 24 + int(19.5 * _math.sin(a))
        fill(px, w, h, ox, oy, tx - 1, ty - 1, tx + 1, ty + 1)


def icon_blocks(px, w, h, ox, oy, frame):
    hline(px, w, h, ox, oy, 8, 39, 42)               # ground
    # three stacked layers, each with a 1px seam line on top for depth
    for y in range(36, 42):
        for x in range(10, 38):
            put(px, w, h, ox + x, oy + y)
    hline(px, w, h, ox, oy, 10, 37, 35)
    for y in range(28, 35):
        for x in range(14, 34):
            put(px, w, h, ox + x, oy + y)
    hline(px, w, h, ox, oy, 14, 33, 27)
    for y in range(20, 27):
        for x in range(18, 30):
            put(px, w, h, ox + x, oy + y)
    hline(px, w, h, ox, oy, 18, 29, 19)
...
def draw_dev_icons(frame):
    ...
    for i, fn in enumerate((
        ...
        lambda o: icon_term(px, w, h, o, 8, frame),
        lambda o: icon_gear(px, w, h, o, 8, frame),
    )):
        fn(i * 128)  # 48px icon + 80px gap, first flush-left, last flush-right
    return img
...
if __name__ == "__main__":
    ...
    save_both("dev-icons.gif", [scale(draw_dev_icons(i)) for i in range(4)], 250)
